...
RSpec.describe Item, type: :model do
  ...
  describe '#find_item_by_price()' do
    it 'finds an item within the given price range' do
      merch_id = create(:merchant).id
      item1 = Item.create!(name: 'Thing 1', description: 'does something ', unit_price: 12.99,
                           merchant_id: merch_id)
      item2 = Item.create!(name: 'Thing 2', description: 'does something', unit_price: 16.99,
                           merchant_id: merch_id)
      item3 = Item.create!(name: 'Thing', description: 'does something ', unit_price: 20.99,
                           merchant_id: merch_id)
      item4 = Item.create!(name: 'Another thing', description: 'does something ', unit_price: 24.99,
                           merchant_id: merch_id)
  ...
  describe '#find_dependent_invoices' do
    it 'finds any invoices where the given item is the only item on the invoice' do
      ...
      InvoiceItem.create!(item_id: item1.id, invoice_id: invoice1.id, quantity: 1, unit_price: 12.99)
      InvoiceItem.create!(item_id: item2.id, invoice_id: invoice2.id, quantity: 3, unit_price: 12.99)
      InvoiceItem.create!(item_id: item1.id, invoice_id: invoice3.id, quantity: 19, unit_price: 12.99)
      InvoiceItem.create!(item_id: item1.id, invoice_id: invoice3.id, quantity: 17, unit_price: 12.99)
      InvoiceItem.create!(item_id: item1.id, invoice_id: invoice2.id, quantity: 3, unit_price: 12.99)
      InvoiceItem.create!(item_id: item2.id, invoice_id: invoice4.id, quantity: 3, unit_price: 12.99)
